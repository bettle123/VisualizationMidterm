function barplot_dep_pr()
{
	var svg = d3.select("body")
	.append("svg")
	.attr("width", map_width)
	.attr("height",map_height)
	.attr("id", "barPlotDepPr");
...
function BarChart(error, data) {
			...
			data.forEach(function(d) {
				d.DEP_Pr = +d.DEP_Pr;
				dataset.push(d.DEP_Pr);
			});
			
			dataset = dataset.sort(d3.descending);
			
			data = data.filter(function(d,i){
				if(d.DEP_Pr>= dataset[19])
				{
					count++;
				}
				return d.DEP_Pr>=dataset[19];
			})
			
			
			data.sort(function(a, b) { return b.DEP_Pr - a.DEP_Pr; });
			...
			var rects = g.selectAll("rect")
			   ...
			   .attr("y", function(d){
				   return yScale(d.DEP_Pr);
			   })
			   .attr("width", barWidth)
			   .attr("height", function(d){
				   return  height- yScale(d.DEP_Pr);
			   })
			   .style("fill", function(d){
				...
				var center_y = yScale(d.DEP_Pr);
				var tmp_value = d.DEP_Pr;
			...
			.attr("font-weight", "bold")
			.attr("text-anchor", "start")
			.text("Top 20 delay Probability of departure airport flights");
